...
const S={host:{ws:null,room:null,token:null},player:{ws:null,hasAnswered:false},games:[],lastQuestionKey:null,timerId:null,lastTickSecond:null,currentRound:null};
...
function audioTone(freq,duration,volume){const C=window.AudioContext||window.webkitAudioContext;if(!C)return;const ctx=new C();const osc=ctx.createOscillator();const gain=ctx.createGain();
osc.type="sine";osc.frequency.value=freq;gain.gain.value=volume;osc.connect(gain);gain.connect(ctx.destination);osc.start();osc.stop(ctx.currentTime+duration/1000);setTimeout(()=>ctx.close(),duration+30);}
...
function startRoundTicker(){if(S.timerId)clearInterval(S.timerId);S.lastTickSecond=null;S.timerId=setInterval(()=>{if(!S.currentRound)return;const msLeft=updateTimerUi("hostTimer","hostTimerBar",S.currentRound);updateTimerUi("playerTimer","playerTimerBar",S.currentRound);
...
if(msLeft<=0&&!S.player.hasAnswered){toggleAnswerButtons(true);byId("result").className="feedback bad";byId("result").textContent="⏰ Time is up for this question.";playTimeout();S.player.hasAnswered=true;}},200);}
...
if(r.selectedGameId)byId("gameSelect").value=r.selectedGameId;const q=r.round.question;const qKey=q?String(q.index)+"-"+String(r.round.endsAt):"none";
if(S.lastQuestionKey!==qKey){S.lastQuestionKey=qKey;S.player.hasAnswered=false;byId("result").className="feedback";byId("result").textContent="Choose your answer before time runs out.";}
...
startRoundTicker();mode("host");loadCatalog().catch(()=>alert("Failed loading games"));
</script></body></html>`;
}
